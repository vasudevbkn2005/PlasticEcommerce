<div>
    <h1 class="text-2xl font-bold mb-4">Category Page</h1>
                    ...
                    @foreach ($categories as $category)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="border px-4 py-2">
                                @if ($category->cimage)
                                    <img src="{{ asset('storage/' . $category->cimage) }}"
                ...
                <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-40">
                    <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                        ...
                        <form wire:submit.prevent="store">
                            <input type="text" wire:model="name" style="text-transform: capitalize;"
                                placeholder="Category name" class="border p-2 mb-4 w-full" required>
                            @error('name')
                            ...
                            @if ($existingImage)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ asset('storage/' . $existingImage) }}"
                                        class="h-32 w-32 rounded object-cover border shadow cursor-pointer"
                                        @click="imageModal = true; modalImage = '{{ asset('storage/' . $existingImage) }}'">
                                </div>
                            @endif

                            <input type="file" wire:model="cimage" class="border p-2 mb-4 w-full"
